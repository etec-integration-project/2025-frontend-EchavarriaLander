...
const MyList = () => {
  ...
  useEffect(() => {
    const fetchList = async () => {
      setLoading(true);
      const ids = JSON.parse(localStorage.getItem('myList') || '[]');
      const results: any[] = [];
      for (const id of ids) {
  ...
  return (
    <>
      ...
      <div className="pt-20 bg-black min-h-screen text-white">
        <h1 className="text-2xl font-bold mb-6 px-4">Mi Lista</h1>
        {loading ? (
          <div className="text-center text-gray-400">Cargando...</div>
        ) : movies.length === 0 ? (
          <div className="text-center text-gray-400">No tienes películas o series en tu lista aún.</div>
        ) : (
